...
<meta name="twitter:image" content="${chartImageUrl}">
...
body { 
    margin:0; background:#0f172a; font-family: 'Segoe UI', Arial, sans-serif; color:#e2e8f0; 
    overflow: hidden; height: 100vh; width: 100vw;
}

#rotate-message {
    display: none; position: fixed; top:0; left:0; width:100%; height:100%;
    background:#0f172a; z-index: 999; flex-direction: column;
...
.btn { 
    background: #1e293b; border: 1px solid #334155; color: #f8fafc; padding: 6px; 
    border-radius: 6px; cursor: pointer; display: flex; align-items: center; text-decoration: none;
}
...
#full-capture-area { 
    ...
    justify-content: center; position: relative; overflow: hidden; background: #0f172a;
}
.chart-container { width: 100%; height: 100%; display: flex; justify-content: center; align-items: center; }
img { max-width: 100%; max-height: 100%; object-fit: contain; }

/* Screenshot box nascosto */
#screenshot-title-box { display: none; position: absolute; top: 10px; left: 10px; }
</style>
</head>
...
<div id="rotate-message">
    ...
    <h2 style="margin-top:20px">Please rotate your device</h2>
</div>
...
<div class="container">
  <div class="header-row">
    ...
    <div class="actions">
      ${prev ? `<a class="btn nav-btn" href="${prev}.html" onclick="saveFsState()"><svg viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6"/></svg></a>` : ''}
      ${next ? `<a class="btn nav-btn" href="${next}.html" onclick="saveFsState()"><svg viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg></a>` : ''}
      ...
      <a class="btn" href="https://twitter.com/intent/tweet?url=${currentUrl}&text=${shareTextX}" target="_blank">
      ...
      <a class="btn" href="https://www.facebook.com/sharer/sharer.php?u=${currentUrl}" target="_blank">
  ...
  <div id="full-capture-area">
    <div id="screenshot-title-box">
        <div style="font-size:22px; font-weight:bold; color:#f8fafc;">${title}</div>
        <div style="font-size:13px; color:#67e8f9;">commoditysupercycle.com</div>
    </div>
...
<script>
// Gestione Fullscreen Persistente
function saveFsState() {
    if (document.fullscreenElement) {
        sessionStorage.setItem('fs_mode', 'true');
    }
}

window.onload = () => {
    if (sessionStorage.getItem('fs_mode') === 'true') {
        sessionStorage.removeItem('fs_mode');
        // Il fullscreen richiede interazione, quindi serve un piccolo trick
        // o l'utente deve cliccare una volta. Ma i browser moderni lo bloccano.
        // Proviamo l'auto-attivazione:
        document.documentElement.requestFullscreen().catch(e => console.log("Auto-FS blocked"));
    }
};
