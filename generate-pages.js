...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  @media (orientation: portrait) and (max-width: 1024px) { #rotate-message { display: flex; } }
  ...
  .btn { background: #1e293b; border: 1px solid #334155; color: #f8fafc; padding: 6px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; text-decoration: none; transition: background 0.2s; }
  ...
  /* SCREENSHOT BOX (Invisibile a schermo) */
  ...
  @media (min-width: 768px) {
    .header-row { 
      min-height: 100px;
      padding: 30px 10% 10px 10%; /* CONTROLLO MANIACALE POSIZIONE */
    }
    ...
    /* Assicurati che i tasti nav siano visibili */
    #prev-btn, #next-btn { display: flex !important; }
  }
  ...
  @media (max-width: 767px) {
    ...
    .title { font-size: 16px; width: 80%; }
    .subtitle { font-size: 11px; }
    .source-inline { font-size: 10px; }
    
    /* Bottoni sovrapposti a destra per risparmiare spazio */
    .actions { 
      position: absolute; 
      right: 8px; 
      top: 8px; 
    ...
    /* Nascondi frecce su mobile (usa lo swipe) */
    #prev-btn, #next-btn, .actions > div { display: none !important; }

    #full-capture-area { padding: 0; }
    .chart-container { justify-content: flex-start; align-items: flex-start; }
...
<div class="container">
  <div class="header-row">
    ...
    <div class="actions">
      <button class="btn" title="Fullscreen" onclick="toggleFullScreen()"><svg viewBox="0 0 24 24"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg></button>
      <button class="btn" title="Screenshot" onclick="takeScreenshot()"><svg viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg></button>
  ...
  <div id="full-capture-area">
    <div id="screenshot-title-box">
      ...
      <div id="ss-source-bottom" style="display:none; margin-top:8px;">
        <span style="color:#f8fafc">Sources:</span> <span id="ss-source-text"></span>
      </div>
    </div>
...
function updatePage() {
  ...
  sInline.innerHTML = validSources.length 
    ? '<span style="color:#f8fafc">Sources:</span> ' + validSources.map(s => '<a href="' + s.link + '" target="_blank">' + s.text + '</a>').join(' · ')
    : '';
  ...
  document.getElementById('next-btn').style.visibility = currentIndex < chartsData.length - 1 ? 'visible' : 'hidden';
...
document.addEventListener('keydown', e => {
  if (e.code === 'ArrowRight' || e.code === 'Space') { e.preventDefault(); navigateNext(); }
  if (e.code === 'ArrowLeft') { e.preventDefault(); navigatePrev(); }
});

// Swipe Logic Professionale
let tsX = 0;
const area = document.getElementById('full-capture-area');
area.addEventListener('touchstart', e => { tsX = e.touches[0].screenX; }, { passive: true });
...
files.forEach(file => fs.writeFileSync(file.replace('.png', '') + '.html', createPage(file)));
console.log("✅ Pagine rigenerate con successo. Bug 'Touch-Laptop' risolto.");