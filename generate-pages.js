...
<div class="container">
  <div class="header-row">
    ...
    <div class="actions">
      ...
      <a class="btn" href="https://twitter.com/intent/tweet?url=${currentUrl}&text=${shareTextX}" target="_blank"><svg viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.045 4.126H5.078z"/></svg></a>
      <a class="btn" href="https://www.facebook.com/sharer/sharer.php?u=${currentUrl}" target="_blank"><svg viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
      
      <div style="width: 1px; height: 20px; background: #334155; margin: 0 4px;"></div>

      ${prev ? `<a id="prev-btn" class="btn nav-btn" href="${prev}.html" onclick="beforeNav()"><svg viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6"/></svg></a>` : ''}
      ${next ? `<a id="next-btn" class="btn nav-btn" href="${next}.html" onclick="beforeNav()"><svg viewBox="0 0 24 24"><path d="M9 18l6-6-6-6"/></svg></a>` : ''}
    </div>
  </div>
...
document.addEventListener('keydown', (e) => {
    if (e.code === 'Space' || e.key === 'ArrowRight') {
        ...
        if (next) { beforeNav(); next.click(); }
    } else if (e.key === 'ArrowLeft') {
        const prev = document.getElementById('prev-btn');
        if (prev) { beforeNav(); prev.click(); }
    }
});

function beforeNav() {
    if (document.fullscreenElement) localStorage.setItem('keepFullscreen', 'true');
}

window.addEventListener('load', () => {
    if (localStorage.getItem('keepFullscreen') === 'true') {
        localStorage.removeItem('keepFullscreen');
        // Tentativo di ripristino Fullscreen (richiede spesso un click)
...
function takeScreenshot() {
    ...
    if(ssSource) ssSource.style.display = 'block';
