...
<body>
  ...
  <div id="screenshot-container" style="display:none; position:absolute; left:-99999px; top:-99999px; background:#1E1E1E; padding:50px 60px; width:1450px;">
    ...
    <img id="ss-image-hidden" style="width:100%; height:auto; display:block; image-rendering:crisp-edges;">
    <div id="ss-source-hidden" style="margin-top:30px; font-size:17px; color:#f8fafc; font-weight:500;"></div>
  </div>
  ...
  <script>
    ...
    /* ====================== SCREENSHOT ROBUSTO (fix definitivo) ====================== */
    function takeScreenshot() {
      ...
      preload.src = 'charts/' + c.file;

      preload.onload = () => {
        hiddenImg.src = preload.src;

        // Doppio requestAnimationFrame per dare tempo al browser di renderizzare
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            ...
            html2canvas(container, { 
              ...
              // Debug: se il dataURL è troppo corto → canvas vuoto
              if (dataUrl.length < 1000) {
                console.error('❌ Canvas vuoto o corrotto!');
                alert('Errore: il canvas è vuoto. Prova ad aprire la pagina con un server locale (npx serve .)');
                return;
              }
...
console.log("🎉 Pagine HTML generate con successo! (Screenshot con doppio requestAnimationFrame + controllo canvas)");