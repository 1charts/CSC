  ...
  <script>
    ...
    function takeScreenshot() {
      ...
      preload.onload = () => {
        ...
        canvas.width = WIDTH;
        canvas.height = PADDING_TOP + TITLE_HEIGHT + SUBTITLE_HEIGHT + preload.height * (WIDTH - 2 * PADDING_X) / preload.width + SOURCES_HEIGHT + PADDING_BOTTOM;
        ...
        // Immagine del chart (centrata)
        const imgX = PADDING_X;
        const imgY = PADDING_TOP + TITLE_HEIGHT + SUBTITLE_HEIGHT;
        ...
        if (validSources.length) {
          ...
          ctx.fillText('Sources:', PADDING_X, imgY + imgH + 55);

          ctx.fillStyle = '#94a3b8';
          ctx.font = '500 22px Segoe UI, Arial';
          const sourcesText = validSources.map(s => s.text).join(' · ');
          ctx.fillText(sourcesText, PADDING_X + 100, imgY + imgH + 55);
        }
